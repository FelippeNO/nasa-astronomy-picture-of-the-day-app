...
void main() {
  ...
  late final GetPicturesFromDateService service;
  ...
  setUpAll(() {
    ...
    service = GetPicturesFromDateService(repository);
  });

  test('Should return a List<PictureEntity> on successful return from the repository', () async {
    when(() => repository.getPicturesFromDate(startDate: any(named: 'startDate')))
    ...
    verify(() => repository.getPicturesFromDate(startDate: tStartDate)).called(1);
    verifyNoMoreInteractions(repository);
  });

  test('Should return a GetPicturesFromDateFailure on unsuccessful return from the repository', () async {
    when(() => repository.getPicturesFromDate(startDate: any(named: 'startDate')))
        .thenAnswer((_) async => Left(GetPicturesFromDateFailure()));
    final Either<CoreFailure, List<PictureEntity>> result = await service.call(startDate: tStartDate);
